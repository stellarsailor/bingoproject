...
export default function BingoCreateInformationPane(props) {
    ...
    const { categoryList, bingoCategory, setBingoCategory, bingoTitle, setBingoTitle, bingoDescription, setBingoDescription } = props
    ...
    return ( 
        <>
            ...
            value={bingoCategory}
            >
                {categoryList.slice(1).map((v, index) => <Option key={v.id} value={v.id}>{v[`name_${i18n.language}`]}</Option>)}
            </Select>
