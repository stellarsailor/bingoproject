...
export default async (req, res) => {
  ...
  if (req.method === "GET") {
    ...
    if (!duplicated) {
      //when the submit is not duplicated recent within 10 minutes
      if (completedMarks !== 0) {
        //and there must be any check on bingo.
        const insertResult = await db.query(escape`
                    INSERT INTO results (bingoId, binaryResult, completedMarks, completedLines, ipAddress)
                    VALUES (${bingoId}, ${JSON.stringify(
          binaryResult
        )}, ${completedMarks}, ${completedLines}, ${ipAddress});
                `);
    ...
    res.status(200).json({
      ...
      percentage: duplicated
        ? null //when the result is duplicated
        : completedMarks === 0
        ? 1
        : percentage[0].percentage, //when user submit 0 mark bingo, it shows 1(will be *100, so 100% percentage)
      results,
    });
    ...
    if (compareUserId[0].TF === 1) {
      const editBingo = await db.query(escape`
                UPDATE bingos SET categoryId = ${category}, title = ${title}, description = ${description}, achievements = ${JSON.stringify(
        achievements
      )}, modifiedAt = now()
                WHERE id = ${bingoId};
            `);
